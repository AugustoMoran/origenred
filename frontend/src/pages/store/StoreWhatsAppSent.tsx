...
export const StoreWhatsAppSent: React.FC = () => {
  const [trackEvent] = useTrackEventMutation();

  useEffect(() => {
    trackEvent({ event: 'whatsapp_checkout_sent' }).catch(() => {});
  }, [trackEvent]);

  return (
    <div className="max-w-lg mx-auto space-y-6 animate-slide-up text-center">
      <SEO title="Consulta enviada" description="Tu pedido fue enviado por WhatsApp" />

      <div className="w-16 h-16 rounded-full bg-[#25D366]/20 flex items-center justify-center mx-auto">
        <svg className="w-8 h-8 text-[#25D366]" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
